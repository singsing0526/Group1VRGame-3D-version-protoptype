...
public class LB_LightingBox : EditorWindow
{
	...
	*/
	...
	#endregion
	SharpConfig.Configuration lb_guide_videos;
	#region Init()
	...
	void OnEnable()
	{
		lb_guide_videos = SharpConfig.Configuration.LoadFromFile(Application.dataPath + "/"
											 + "LightingBox2/Resources/Config/LB_Video_Guides.txt");
	...
	void OnGUI()
	{
		...
		GUILayout.Label ("Lighting Box 2 - Unity 2020.3.5f1 update 2.9 May 2021", EditorStyles.helpBox);
		...
		if (!helpBoxVideo)
		{
			if (GUILayout.Button("Show Video Help"))
			{
				helpBoxVideo = !helpBoxVideo;
			}
		}
		else
		{
			if (GUILayout.Button("Hide Video Help"))
			{
				helpBoxVideo = !helpBoxVideo;
			}
		}
		*/
		...
		{
			Application.OpenURL("https://assetstore.unity.com/packages/tools/utilities/lighting-box-2-next-gen-lighting-solution-2021-93057?_gl=1*snrlct*_ga*MTA3OTkzMzI0Ny4xNjIzNjc3NTEy*_ga_1S78EFL1W5*MTYyMzc0MDExMi4xMC4xLjE2MjM3NDExMzcuMjM.&_ga=2.161879545.459844169.1623677512-1079933247.1623677512&_gac=1.47585749.1619901400.Cj0KCQjw-LOEBhDCARIsABrC0Tk2P0OGjpM1touQZGuEpjx_8LT092-G3TZIVKigLZcc5GZs6oyg4gMaAvaKEALw_wcB");
		}			
		...
		if (winMode == WindowMode.Part1) {
			...
			if (profileState) {
					...
					if (GUILayout.Button("Apply", GUILayout.Width(43), GUILayout.Height(17)))
					{
						if (autoProfile == AutoProfile.Default)
						{
							AssetDatabase.CopyAsset(AssetDatabase.GetAssetPath
								(Resources.Load("Config/Default")), EditorPrefs.GetString(EditorSceneManager.GetActiveScene().name));
						...
						/*
						if (autoProfile == AutoProfile.ArchViz)
						{
							AssetDatabase.CopyAsset(AssetDatabase.GetAssetPath
								(Resources.Load("Config/ArchViz")), EditorPrefs.GetString(EditorSceneManager.GetActiveScene().name));
						...
						if (autoProfile == AutoProfile.Games)
						{
							AssetDatabase.CopyAsset(AssetDatabase.GetAssetPath
								(Resources.Load("Config/Games")), EditorPrefs.GetString(EditorSceneManager.GetActiveScene().name));
						...
						if (autoProfile == AutoProfile.WebGL)
						{
							AssetDatabase.CopyAsset(AssetDatabase.GetAssetPath
								(Resources.Load("Config/WebGL")), EditorPrefs.GetString(EditorSceneManager.GetActiveScene().name));
				...
				if (helpBox)
					EditorGUILayout.HelpBox("Show WebGL2.0 target tips", MessageType.Info);
			...
			if (ambientState) {
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Ambient- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Ambient"].StringValue);
				}
				if (helpBox)
				{
			...
			if (sunState) {				
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Sun- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Sun"].StringValue);
				}
				if (helpBox)
			...
			if (lightSettingsState) {
				if (helpBoxVideo)
				{ 
					if (GUILayout.Button("Watch -Scene- video guide"))
					Application.OpenURL(lb_guide_videos["Guide Videos"]["Scene"].StringValue);
				}
			...
			if (helpBox)
				EditorGUILayout.HelpBox ("Activate / Diactivate shadows for point , spot and directional lights", MessageType.Info);
			...
			if (buildState) {
				...
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Build- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Build"].StringValue);
				}
				if (helpBox)
			...
			if(vLightState)
						EditorGUILayout.HelpBox ("Volumetric Lights is only available in full version", MessageType.Warning);
			...
			if(sunShaftState)
			{
				...
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Sun Shaft- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Sun Shaft"].StringValue);
				}
			}
			...
			if(fogState)
			{
				...
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Global Fog- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Global Fog"].StringValue);
				}
			}
			...
			if(dofState)
			{
				...
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Depth of Field- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Depth of Field"].StringValue);
				}
			}
			...
			if(bloomStateRef != bloomState)
			{
				...
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Bloom- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Bloom"].StringValue);
				}
			}
			...
			if(colorState)
			{
				...
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Color Grading- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Color Grading"].StringValue);
				}
			}
			...
			if (colorState)
			{
				...
				helper.Update_ColorGrading(colorMode, exposureIntensity, contrastValue, temp, exposureCompensation, saturation, colorGamma, colorLift, gamma, lut, exposureMode, eyeMin, eyeMax);
			...
			if(snowState)
					EditorGUILayout.HelpBox ("Snow is only available in full version", MessageType.Warning);
			...
			{
				if (helpBox)
					EditorGUILayout.HelpBox ("Activate Antialiasing for the camera , preferred to use TAA for next-gen platforms", MessageType.Info);
			...
			if(aoState)
				EditorGUILayout.HelpBox("AO is only available in full version", MessageType.Warning);
			...
			if(ssrStateRef != ssrState)
			{
				...
				{
					if (helpBox)
						EditorGUILayout.HelpBox ("Activate internal Screen Space Reflections effect for your camera", MessageType.Info);
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -SSR- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["SSR"].StringValue);
				}
			}
			...
			if(st_ssrState )
				EditorGUILayout.HelpBox("Stochastic SSR is only available in full version", MessageType.Warning);
