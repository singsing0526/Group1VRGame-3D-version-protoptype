...
public class LB_LightingBoxHelper : MonoBehaviour {
	...
	void UpdatePostEffects(LB_LightingProfile profile)
	{
		...
		// Color Grading
		helper.Update_ColorGrading(profile.colorMode, profile.exposureIntensity, profile.contrastValue, profile.temp, profile.exposureCompensation, profile.saturation, profile.colorGamma, profile.colorLift, profile.gamma, profile.lut, profile.exposureMode, profile.eyeMin, profile.eyeMax);
	...
	public void Update_ColorGrading(ColorMode colorMode, float exposureIntensity, float contrastValue, float temp, float exposureCompensation
		, float saturation, Color colorGamma, Color colorLift, float gamma, Texture lut,LB_ExposureMode exposureMode, float eyeMin,float eyeMax)
	{
		ColorGrading cg;
